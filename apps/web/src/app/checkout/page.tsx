...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const publishableKey = process.env.NEXT_PUBLIC_MOYASAR_PUBLISHABLE_KEY || 'pk_test_replace_me';
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#090d16] text-white px-4">
        <div className="max-w-md w-full text-center bg-[#111827] border border-zinc-800 rounded-2xl p-8 shadow-xl">
          ...
          <p className="text-zinc-400 text-sm mb-6">الدورة التي تبحث عنها غير موجودة أو تم إلغاؤها.</p>
          <Link href="/courses" className="inline-block bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-6 py-2.5 rounded-lg transition-colors">
  ...
  return (
    <main className="min-h-screen bg-[#090d16] text-white font-sans selection:bg-indigo-500 selection:text-white" dir="rtl">
      ...
      <div className="max-w-6xl mx-auto px-4 py-12 relative z-10">
        ...
        <div className="flex items-center gap-2 text-sm text-zinc-400 mb-8">
          <Link href="/courses" className="hover:text-indigo-400 transition-colors">الدورات</Link>
          <ChevronRight className="w-4 h-4 rotate-180" />
          <Link href={`/courses/${course.id}`} className="hover:text-indigo-400 transition-colors">{course.title}</Link>
          <ChevronRight className="w-4 h-4 rotate-180" />
          ...
          <div className="lg:col-span-7">
            <div className="bg-[#111827]/80 backdrop-blur-md border border-zinc-800 rounded-2xl p-6 shadow-2xl">
              <div className="mb-6">
                ...
                <p className="text-zinc-400 text-sm">أدخل تفاصيل بطاقة الدفع (MADA أو Visa أو MasterCard) لإتمام عملية الشراء.</p>
              </div>
          ...
          <div className="lg:col-span-5">
            <div className="bg-gradient-to-br from-[#1e1b4b]/40 to-[#111827]/90 backdrop-blur-md border border-indigo-950 rounded-2xl p-6 shadow-2xl sticky top-8">
              <h2 className="text-lg font-bold mb-6 pb-4 border-b border-zinc-800/80 flex items-center justify-between">
                ...
                <span className="text-xs font-normal text-indigo-400 bg-indigo-500/10 px-2.5 py-1 rounded-full border border-indigo-500/20">خطوة واحدة متبقية</span>
              </h2>
              ...
              <div className="space-y-6">
                ...
                <div className="flex gap-4">
                  {course.thumbnailUrl && (
                    <img src={course.thumbnailUrl} alt={course.title} className="w-24 h-16 object-cover rounded-lg border border-zinc-850" />
                  )}
                  <div>
                ...
                <div className="bg-[#0b0f19] border border-zinc-900 rounded-xl p-4 space-y-3">
                  <div className="flex justify-between text-sm text-zinc-400">
                    ...
                    <span className="line-through">{Math.round(Number(course.price) * 1.5)} ر.س</span>
                  </div>
                ...
                <div className="border-t border-zinc-800/80 pt-4 space-y-3">
                  <h4 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">تفاصيل الحساب</h4>

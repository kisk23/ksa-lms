...
const TAUGHT_COURSES = [
  { id: '1', title: 'أساسيات البرمجة بلغة بايثون', grade: 'الصف الأول الثانوي', students: 42, activeLessons: 12 },
  { id: '2', title: 'مبادئ الذكاء الاصطناعي والآلة', grade: 'الصف الثاني الثانوي', students: 38, activeLessons: 8 },
];

const PENDING_SUBMISSIONS = [
  { id: 'sub-1', studentName: 'أحمد محمد', taskTitle: 'الواجب العملي: كتابة دوال التكرار', courseTitle: 'أساسيات البرمجة بلغة بايثون', submittedAt: 'منذ ساعتين' },
  { id: 'sub-2', studentName: 'سارة خالد', taskTitle: 'تحليل البيانات باستخدام مكتبة Pandas', courseTitle: 'مبادئ الذكاء الاصطناعي والآلة', submittedAt: 'منذ 4 ساعات' },
  { id: 'sub-3', studentName: 'فيصل العتيبي', taskTitle: 'الواجب العملي: كتابة دوال التكرار', courseTitle: 'أساسيات البرمجة بلغة بايثون', submittedAt: 'منذ يوم' },
];
  ...
  return (
    <div className="space-y-8 font-sans text-right" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-on-background">أهلاً بك، {user?.name || 'أ. أحمد'} 👋</h1>
        ...
        {activeTab === 'classes' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {TAUGHT_COURSES.map((course) => (
              <div key={course.id} className="border border-gray-100 rounded-2xl p-6 flex flex-col justify-between hover:shadow-xs transition-shadow bg-surface-container-lowest">
                <div>
                  <span className="text-[10px] bg-primary/5 text-primary px-2.5 py-1 rounded-full font-bold">
                    {course.grade}
                  </span>
                  <h3 className="text-base font-bold text-on-background mt-3 mb-2">{course.title}</h3>
          ...
          <div className="space-y-4">
            {PENDING_SUBMISSIONS.map((sub) => (
              <div key={sub.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-surface-container-lowest border border-gray-100 rounded-2xl gap-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-right">
                    <h4 className="text-xs font-bold text-on-background">تسليم جديد من: {sub.studentName}</h4>
                    <p className="text-[11px] text-on-surface-variant font-semibold mt-1">
                      الواجب: {sub.taskTitle} • <span className="text-primary">{sub.courseTitle}</span>
                    </p>
                  </div>
                </div>

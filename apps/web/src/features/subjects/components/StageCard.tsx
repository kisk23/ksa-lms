...
export function StageCard({ title, description, onClick, isActive = true }: StageCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      ...
      {/* Description */}
      <p className="flex-1 text-sm leading-7 text-on-surface-variant">{description}</p>

      {/* Footer */}
      <div className="mt-8 flex items-center justify-between border-t border-outline-variant pt-4">
        <span className="text-sm font-medium text-primary-container">اختر المرحلة</span>

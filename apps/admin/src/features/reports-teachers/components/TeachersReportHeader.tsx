...
import type { SubjectCategory, DateRange } from '../types';

interface TeachersReportHeaderProps {
  onSubjectChange: (subject: SubjectCategory) => void;
  onDateRangeChange: (range: DateRange) => void;
}

export function TeachersReportHeader({
  onSubjectChange,
  onDateRangeChange,
}: TeachersReportHeaderProps) {
  return (
    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8">
      <div className="space-y-4">
        ...
        <div>
          <h1 className="text-5xl font-bold text-[#0F172A]">تحليلات أداء المعلمين</h1>
          <p className="text-xl text-[#64748B] mt-2 max-w-2xl leading-relaxed">
            نظرة شاملة على أداء أفضل المعلمين والمقاييس الرئيسية.
          </p>
        </div>
      </div>
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative">
          ...
          <ChevronDown className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none w-5 h-5" />
        </div>
